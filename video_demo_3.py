...
class YOLO(object):
    ...
    def run(self, frame, frames):
        if self.skip_flag == 0:
            ...
            for i in range(output.shape[0]):
                data_list = np.array([[frames, int(output[i, 1]), int(output[i, 3]), int(output[i, 2]), int(output[i, 4]), classes[int(output[i, 7])]]])
...
def fetch_base(videofile, savefile):
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            print("FPS of the video is {:5.2f}, run time: {:5.2f}".format(1/ (time.time() - pre_time), time.time() - pre_time))
            key = cv2.waitKey(1)
            if key & 0xFF == ord('q'):
                break
            ...
            base = pd.DataFrame({'frame':yolo.data[:,0], 'x0':yolo.data[:,1], 'x1':yolo.data[:,2], 'y0':yolo.data[:,3], 'y1':yolo.data[:,4], 'class':yolo.data[:,5]})
...
def acc_skip_test():
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            f1_avg_val = np.average(f1_list)

            f1_avg_list = np.append(f1_avg_list, f1_avg_val)

            f1_df = pd.DataFrame({'f1_score': f1_ind_list})

            f1_df_1 = pd.DataFrame({'f1_1_sec_avg': f1_avg_list})



            f1_df.to_csv('416_f1_hw.csv')
            f1_df_1.to_csv('416_f1_avg_hw.csv')
            #print('f1 size:{}, f1 list:{}'.format(len(f1_ind_list), f1_ind_list))
            #print('size:{}, f1 list:{}'.format(len(f1_avg_list), f1_avg_list))
            break
